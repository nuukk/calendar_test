...
HOLIDAYS <- as.Date(c("2025-12-25","2026-01-01","2026-02-16","2026-02-17",
                      "2026-02-18","2026-03-02","2026-05-05","2026-05-25",
...
normalize_events_df <- function(df) {
  ...
  if (!"main_category" %in% names(df)) df$main_category <- NA_character_
  if ("main_category" %in% names(df)) df$main_category <- as.character(df$main_category)
    ...
    card(
      ...
      card_body(
        selectizeInput(
          ...
          label=div(bs_icon("folder2-open"), "카테고리(대분류)"),
          choices=character(0),
          selected=character(0),
          multiple=TRUE,
          options=list(
            plugins=list("remove_button"),
            placeholder="전체 (선택 없으면 전체)"
          )
        ),
        selectInput(
          "category_filter",
